...
export const getBstatsMaxValue = async (chartId: string): Promise<number> => {
    const resp = await fetch(`${bstatsBaseUrl}/plugins/${bstatsPluginId}/charts/${chartId}/data?maxElements=${maximumDataElements}`, {
        headers: {
    ...
    if (!resp.ok) {
        throw new Error(`Failed to fetch chart data ${chartId} for plugin ${bstatsPluginId} from bstats: ${resp.status} ${resp.statusText}`);
    }
